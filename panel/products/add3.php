...
if(!isset($_SESSION['gov']['id']) || $_SESSION['gov']['admin']==0){
	...
	if(!isset($_GET['category']) || !isset($_GET['seccion'])){
...
?>
	...
	<section class="content-add">
		<section class="add">
			<div class="add-title"><img src="../img/regresar.png" style="cursor: pointer; margin-right: 20px;" width="70px" height="20px" onclick="window.history.back();">¿EN QUE ESTADO ESTA EL PRODUCTO?<br><br><hr></div>
			<nav class="content-nav">
				<li class="nav-op" onclick="window.location='add4.php?category=<?php echo $_GET['category'] ?>&seccion=<?php echo $_GET['seccion']; ?>&status=Nuevo'">
					<div class="nav-title">Nuevo</div>
				</li>
				<li class="nav-op" onclick="window.location='add4.php?category=<?php echo $_GET['category'] ?>&seccion=<?php echo $_GET['seccion']; ?>&status=Usado'">
					<div class="nav-title">Usado</div>
				</li>
				<li class="nav-op" onclick="window.location='add4.php?category=<?php echo $_GET['category'] ?>&seccion=<?php echo $_GET['seccion']; ?>&status=Reacondicionado'">
					<div class="nav-title">Reacondicionado</div>
				</li>
			</nav>
