...
?>
	...
	<section class="content-add">
		<section class="add">
			<div class="add-title"><img src="../img/regresar.png" style="cursor: pointer; margin-right: 20px;" width="70px" height="20px" onclick="window.history.back();">¿A QUE SECCIÓN PERTENECE?<br><br><hr></div>
			<nav class="content-nav">
				<?php
					$rs=mysqli_query($conexion,"SELECT * FROM seccion WHERE category='".$_GET['category']."'");
					while($row=mysqli_fetch_array($rs)){
						?>
						<li class="nav-op" onclick="window.location='add3.php?category=<?php echo $_GET['category'] ?>&seccion=<?php echo $row['seccion']; ?>'">
							<div class="nav-title"><?php echo $row['seccion']; ?></div>
						</li>
						<?php
					}
				?>
